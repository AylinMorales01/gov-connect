...
- `docs/` — architecture documentation (Spanish); **partially stale**, see Gotchas
...
**Dual-database split (ADR-007):** SQL Server is the transactional/operational store; DuckDB is the analytical engine. They are kept in sync by an ETL pipeline: `EtlService.runFullEtl()` → `ExportService` dumps `collections`, `departments`, `budgets` to CSV under `exports/` → `ImportService` loads each CSV into DuckDB via `CREATE OR REPLACE TABLE ... AS SELECT * FROM read_csv_auto(...)`. New analytics features must run the ETL (or extend it) before data exists to query. ETL is triggered manually via `POST /analytics/etl/run`.
...
Base path `/api/v1`:
- `GET /dashboard/summary`, `/dashboard/monthly-collections`, `/dashboard/expiring-contracts`, `/dashboard/budget-execution`
- `GET /analytics/health`, `/analytics/monthly-trend`, `/analytics/financial-overview`, `/analytics/department-ranking`
- `POST /analytics/etl/run` — manual ETL trigger
...
- `SecurityConfig` currently permits all `/api/v1/dashboard/**` and `/api/v1/analytics/**` plus Swagger paths; the rest requires authentication (not yet enforced — JWT is a roadmap item, not implemented). CORS is scoped to localhost:5173 / 3000.
...
- `pages/` — route views; only `DashboardPage` is fully implemented; `AnalyticsPage`, `ContractsPage`, `AutomationPage` are stubs
...
- **Database name:** `database/sqlserver/01-create-database.sql` creates `GOV_CONNECT_DB`, matching `backend/src/main/resources/application.yaml`. The `06-demo-data.sql` script also targets `GOV_CONNECT_DB`.
...
- **`docs/` is partially stale** relative to the code: it lists Spring Boot 3 (the pom uses the Spring Boot 4.x parent and `spring-boot-starter-webmvc`), marks DuckDB and React as upcoming (both are implemented), and `docs/06-API.md` shows `/dashboard/contracts-expiring` while the implemented route is `/dashboard/expiring-contracts`. ADRs remain the authoritative design rationale; the code is the source of truth for endpoints.
